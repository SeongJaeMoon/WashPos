...
public class InputField extends JPanel {
    ...
    private static final int LABEL_SIZE = 20;    // label font size
    ...
    public InputField(JComponent input, String text, int isize, int lsize, int width, boolean labelOnTop) {
        ...
        LABEL.setFont(MyFont.BOLD_FONT.deriveFont((float)lsize));
    ...
    public void errorDetail(boolean show, String text) {
    	if (INPUT instanceof JTextField) {
    		LABEL.setText(show ? text : "전화번호('-'없이 입력해주세요.)");
    	}
    	if (INPUT instanceof JPasswordField) {
    		LABEL.setText(show ? text : "비밀번호 확인");
    	}
    }
